...
function Library:SeraCallback(Function, ...)
	if not Function then
		return
	end
	...
	local Success, Event = pcall(Function, ...)
		...
		if not i then
			return Library:Notify({
				Title = "Error Caught",
				Content = "Screenshot and report this on #bug-reports",
				SubContent = Event,
				Duration = 5,
			})
		end
...
function TabModule:New(Title, Icon, Parent)
	...
	Tab.Frame = New("TextButton", {
		...
		-- 	Position = UDim2.fromOffset(8, 0), -- Adjust the position as needed
		-- }),
	})
	...
	Tab.Sections = {} -- Add this line when creating the Tab
	...
	function Tab:AddSection(SectionTitle)
		...
		table.insert(Tab.Sections, SectionTitle) -- Add the section to the tab's list
...
function Element:New(Idx, Config)
	...
	if not Dropdown.Search then
		SearchTextBox.TextEditable = false
	...
	Creator.AddSignal(UserInputService.InputBegan, function(Input)
		...
		then
			...
			then
				if Dropdown.Search and SearchTextBox.Text ~= "" then
	...
	function Dropdown:BuildDropdownList(str)
		...
		if Dropdown.Limit then
			local new = {}
			for Idx, Value in next, Values do
				if Idx < Dropdown.Limit then
					new[Idx] = Value
				elseif str ~= "" and string.find(string.upper(Value), string.upper(str)) then
	...
	AddSignal(SearchTextBox.FocusLost, function(enter)
		if not enter then
			return
		end
	...
	AddSignal(SearchTextBox:GetPropertyChangedSignal("Text"), function()
		local strtext = SearchTextBox.Text
		if #strtext ~= 0 then
			DropdownDisplay.Text = ""
		else
...
function Element:New(Idx, Config)
	...
	function Slider:SetValues(tb)
		assert(tb.Min, 'Slider New Values - Missing Minimum Value')
		assert(tb.Max, 'Slider New Values - Missing Maximum Value')
